...
async function pennylaneSnapshot(force) {
  ...
  try {
    ...
    try {
      ...
      const invNum = (mi) => { // extrait le n° de facture d'un matched_invoice quel que soit le nom du champ
        if (!mi || typeof mi !== "object") return "";
        return String(mi.invoice_number || mi.number || mi.reference || mi.label || (mi.invoice && (mi.invoice.invoice_number || mi.invoice.number)) || "").trim();
      };
      do {
        const d3 = await plGet("/transactions?limit=100" + (tcur ? "&cursor=" + encodeURIComponent(tcur) : ""));
        const items = d3.items || d3.transactions || [];
        if (!txRaw3 && items.length) txRaw3 = items.slice(0, 3);
        for (const t of items) {
          const td2 = String(t.date || t.execution_date || "").slice(0, 10);
    ...
    } catch (e) { txError = String((e && e.message) || e).slice(0, 140); }
    ...
    PL_CACHE = { at: Date.now(), treso: Math.round(treso * 100) / 100, ca: Math.round(enc * 100) / 100,
      caMois: mensuel[moisK] || 0, caPrevMois: mensuel[prevK] || 0, mensuel, lastPaid, paidRecent, bankTx, txError, txRaw3,
...
app.post("/api/missions", auth, (req, res) => {
  ...
  if (b.op === "del") { o.missions = o.missions.filter((m) => m.id !== b.id); }
    ...
    o.missions.push({ id: "M" + Date.now().toString(36), at: Date.now(), client: S(b.client, 60), nom: S(b.nom, 90), offre: S(b.offre, 40) || "Influence",
      ...
      contexte: S(b.contexte, 2000), recurrence: S(b.recurrence, 30), marque: S(b.marque, 40),
    ...
    ["client","nom","offre","roleCeo","next","echeance","sante","statut","debut","fin","phase","recurrence","marque"].forEach((k) => { if (b[k] !== undefined) m[k] = S(b[k], 140); });
  ...
  const list = USERS.filter((u) => !COPILOT.departed.includes(String(u.email).toLowerCase()) && normName(u.name) !== "kendia")
    ...
    .map((u) => {
      ...
      return { name: u.name, role: u.role === "supervisor" ? "direction" : (u.role === "team" ? "social média" : "cheffe de projet"),
        email: u.email, tel: pr.tel || "", iban: pr.iban || "", notes: pr.notes || "", arrivee: pr.arrivee || "", contrat: pr.contrat || "", nomComplet: pr.nomComplet || "", adresse: pr.adresse || "",
...
app.post("/api/rh/profil", auth, (req, res) => {
  ...
  if (req.body?.nomComplet !== undefined) pr.nomComplet = String(req.body.nomComplet).slice(0, 80);
  if (req.body?.adresse !== undefined) pr.adresse = String(req.body.adresse).slice(0, 200);
